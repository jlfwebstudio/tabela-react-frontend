...
function App() {
  ...
  const exportToExcel = useCallback(() => {
    ...
    const ws = XLSX.utils.json_to_sheet(filteredForExport, { header: defaultTableHeaders }); // Usa defaultTableHeaders para a ordem
    ...
    const overdueRowStyle = {
      font: { color: { argb: "FFFFFFFF" }, name: "Calibri", sz: 11 }, // Texto branco
    ...
    const dueTodayRowStyle = {
      font: { color: { argb: "FF000000" }, name: "Calibri", sz: 11 }, // Texto preto
    ...
    for (let R = range.s.r + 1; R <= range.e.r; ++R) { // Começa da linha 1 (dados)
      ...
      let currentRowBaseStyle = { ...defaultRowStyle }; // Estilo padrão: azul clarinho
      ...
      for (let C = range.s.c; C <= range.e.c; ++C) {
        ...
        // Formatação de Data Limite (DD/MM/YYYY)
          ...
          const dateObj = parseDateForComparison(originalRowData[header]);
          if (dateObj && !isNaN(dateObj)) {
            cell.v = XLSX.utils.date_to_num(dateObj); // Converte para número de série do Excel
